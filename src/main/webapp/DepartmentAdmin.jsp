...
<head>
    ...
    <title>Department Admin Page</title>
...
<div id="notificationSidebar" class="notification-sidebar">
    ...
    <div class="notification-list">
        <c:forEach var="notification" items="${sessionScope.notifications}">
            <div class="notification-item ${notification.departmentRead ? 'departmentRead' : ''}" onclick="openComplaint(${notification.id})">
                <strong>${notification.type}</strong>
                <p><i>${notification.area}, ${notification.address}</i></p>
               <div class="notification-date-time">
                                   <p>${formattedDates[notification.id]}</p>
                               </div>
            </div>
        </c:forEach>
    </div>
 ...
 <div class="container mt-5 mb-5 d-flex justify-content-center">
            <div class="card px-5 py-4 bg-light w-100 shadow-lg p-3 mb-1 bg-white rounded" style="max-width: 600px;">
                <div class="mb-1 mt-3">
                    <a class="dropdown-item fs-2" href="addEmployee">Add Employees</a>
                </div>
            </div>
        </div>
<div class="container mt-5 mb-5 d-flex justify-content-center">
            <div class="card px-5 py-4 bg-light w-100 shadow-lg p-3 mb-1 bg-white rounded" style="max-width: 600px;">
                <div class="mb-1 mt-3">
                    <a class="dropdown-item fs-2" href="departmentAdminViewComplaints">View Complaints</a>
                </div>
            </div>
        </div>
    ...
    <div class="container mb-5  d-flex justify-content-center">
            <div class="card px-5 py-4 bg-light w-100 shadow-lg p-3 mb-5 bg-white rounded" style="max-width: 600px;">
                <div class="mb-1 mt-3">
        <a class="dropdown-item fs-2" href="/xworkzProject/downloadDepartmentComplaints">Download Complaints</a>
                </div>
            </div>
        </div>
...
<script>
  ...
  function openComplaint(complaintId) {
         ...
         fetch('/xworkzProject/markDepartmentNotificationAsRead?notificationId=' + complaintId)
             ...
             .then(result => {
                 if (result === 'success') {
                     ...
                     window.location.href = '/xworkzProject/viewDepartmentComplaints?id=' + complaintId;
                 } else {
     ...
     function fetchNotifications() {
             fetch('/xworkzProject/getDepartmentNotifications')
